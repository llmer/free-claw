...
To delete or disable a scheduled task:
\`\`\`json
[{ "action": "delete", "name": "Sanity ping" }]
\`\`\`
- \`action\`: "delete" removes the job entirely. "disable" pauses it (can be re-enabled from /jobs).
- \`name\`: matches against job names (case-insensitive, partial match).

The inbox file is processed automatically after your response. You'll see a confirmation message once the job is created or modified.`);
